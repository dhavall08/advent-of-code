...
function getPress(line) {
  const [x1, y1, x2, y2, prevX, prevY] = getXY(line);
  const x = prevX + 10000000000000;
  const y = prevY + 10000000000000;
  ...
  return ansA > 0 && Number.isInteger(ansA) && ansB > 0 ? ansA * 3 + ansB : 0;
}
